...
- The alert layer reads `worker_run_history` and storage capacity checks so the local UI and Discord bot can show the
  ...
  evidence. The local manager reads that history through source/status filters, state/severity badges, and offset
  pagination, and can export the filtered rows to CSV for local review notes. Admin notes and resolution comments are
  stored separately in `system_alert_notes` so the incident trail stays attached to the alert record, and the local
  manager can open a per-alert detail panel for state review, acknowledge/snooze actions, note history, and inline
