...
- The alert layer reads `worker_run_history` and storage capacity checks so the local UI and Discord bot can show the
  ...
  evidence. The local manager reads that history through source/status/severity filters, title/message search,
  newest/oldest/severity-first sorting, quick review presets, state/severity badges, and offset pagination, and can
  export the filtered rows to CSV for local review notes. Admin notes and resolution comments are stored separately
  in `system_alert_notes` so the incident trail stays attached to the alert record, and the local manager can open a
...
- `!pubg-alerts` returns current storage and worker alerts for admins. When alert channel IDs are configured in the
  ...
  `!pubg-alert-ack alert_id` or `!pubg-alert-snooze alert_id [minutes]` directly from Discord. Admins can also append
...
- The isolated combined fault matrix binds that passed combined run and every upstream fingerprint to a fixed ordered
  set of four deterministic failures. It injects one post-DELETE failure only in generated MySQL temporary tables and
