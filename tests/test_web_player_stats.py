...
class WebPlayerStatsTests(unittest.TestCase):
    def test_local_manager_uses_registered_player_weapon_and_match_catalogs(self) -> None:
        ...
        self.assertIn('<option value="damage_per_10_minutes">10분당 준 피해</option>', body)
        self.assertIn('"전투 템포와 효율"', body)
        self.assertIn('damage_per_fight: { label: "교전당 준 피해"', body)
        ...
        self.assertIn("analysis.weapon_code === activeWeaponDetail.weapon_code", body)
        self.assertIn("다른 무기에 장착된 파츠는 포함하지 않습니다", body)
        self.assertIn("data-weapon-attachment-min-matches", body)
        self.assertIn("data-weapon-combination-min-matches", body)
        self.assertIn("상한 제한 없음", body)
        self.assertIn('data-attachment-group="', body)
        self.assertIn("부착 부위별 개별 파츠", body)
        self.assertIn("95% 신뢰구간", body)
