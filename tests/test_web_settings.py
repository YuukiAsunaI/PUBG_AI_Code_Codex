...
class WebSettingsTests(unittest.TestCase):
    def test_index_includes_storage_and_collector_settings_forms(self) -> None:
        ...
        self.assertIn("알림에서 작업 실행 상세를 불러왔습니다.", body)
        ...
        self.assertIn("읽기 전용 영향 미리보기를 불러오는 중", body)
        self.assertIn("체크섬 검증: 수행 안 함", body)
        self.assertIn("renderDataDeletionConfirmationState", body)
        self.assertIn("변경 불가 스냅샷 저장", body)
        self.assertIn("확인 기록 저장", body)
        self.assertIn("지문값에 연결된 확인 기록을 저장했습니다.", body)
        ...
        self.assertIn("읽기 전용 사전 점검 계획 생성", body)
        self.assertIn("읽기 전용 사전 점검 계획을 기록했습니다.", body)
        self.assertIn("renderDataDeletionBackupReadiness", body)
        self.assertIn("선택 실행 백업 결과 생성기", body)
        self.assertIn("백업 결과 생성", body)
        ...
        self.assertIn("체크섬 계산: 예", body)
        self.assertIn("복원·격리·삭제: 아니요", body)
        self.assertIn("변경 불가 증거 기록", body)
        self.assertIn("실제 실행 없는 모의 절차", body)
        self.assertIn("실행 없는 모의 절차를 기록했습니다.", body)
        self.assertIn("모의 체크섬 재계산·복원 작업: 아니요", body)
        self.assertIn("격리된 통합 장애 행렬", body)
        self.assertIn("MySQL 임시 테이블 장애 1개 + 합성 격리 장애 3개", body)
        ...
        self.assertIn("정의된 장애 시나리오", body)
        self.assertIn("삭제 검토 패킷", body)
        self.assertIn("변경 불가 JSON 감사 패킷: 예", body)
        self.assertIn("삭제 요청", body)
        ...
        self.assertIn("승인 부여·실행 준비 승격: 아니요", body)
        self.assertIn("내보낸 검토 패킷 검증", body)
        self.assertIn('id="exportedReviewPacketVerifierForm"', body)
        self.assertIn('type="file"', body)
        self.assertIn("현재 MySQL 체인 대조", body)
        self.assertIn("업로드 원문 미보존", body)
        ...
        self.assertIn("검토 패킷 비교", body)
        self.assertIn('id="exportedReviewPacketComparerForm"', body)
        self.assertIn("기준 JSON", body)
        self.assertIn("후보 JSON", body)
        self.assertIn("두 패킷 모두 현재 MySQL 체인과 대조", body)
        self.assertIn("비교 결과 미보존", body)
        ...
        self.assertNotIn('data-deletion-action="execute"', body)
        self.assertIn("reviewDataDeletionRequest", body)
        self.assertIn("승인은 권한 기록만 남기며 실제 삭제 실행은 비활성화", body)
